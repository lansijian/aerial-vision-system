...
class ControlActor:
    def __init__(self, actor_id):
        ...
        self.subtarget_length = 1  # 初始化为1，避免除零错误
        self.subtarget_pos = []
        self.arrive_count = 0
        self.escape_suce_flag = False
        
        # 初始化避免目标为当前位置（防止异常值）
        self.avoid.x = float(self.id) * 5.0
        self.avoid.y = float(self.id) * 2.0
        ...
        # 初始化actor位置（避免服务调用失败时归零）
        self.current_pose = Point()
        self.current_pose.x = float(self.id) * 8.0  # 给每个actor分散的初始位置
        self.current_pose.y = float(self.id) * 3.0
        self.current_pose.z = 1.25
        self.last_pose = Point()
        self.service_fail_count = 0
        ...
        try:
            # 使用 rospy.ServiceProxy 的正确调用方式（关键字参数）
            test_response = self.gazeboModelstate(model_name='actor_' + self.id, relative_entity_name='ground_plane')
            print(f'Actor {self.id}: 初始位置 ({test_response.pose.position.x:.1f}, {test_response.pose.position.y:.1f}, {test_response.pose.position.z:.1f})')
            # 更新初始位置
            self.current_pose.x = test_response.pose.position.x
            self.current_pose.y = test_response.pose.position.y
            self.current_pose.z = test_response.pose.position.z
        except Exception as e:
            print(f'Actor {self.id}: 服务测试失败，使用默认位置: {e}')
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if not int(self.id) in self.left_actors:
                ...
                break
            try:
                # 使用关键字参数调用服务（正确的ROS服务调用方式）
                get_actor_state = self.gazeboModelstate(model_name='actor_' + self.id, relative_entity_name='ground_plane')
                
                self.last_pose = copy.deepcopy(self.current_pose)
                self.gazebo_actor_pose = get_actor_state.pose.position
                self.current_pose = copy.deepcopy(self.gazebo_actor_pose)
                self.service_fail_count = 0  # 重置失败计数
            except Exception as e:
                self.service_fail_count += 1
                if self.service_fail_count <= 3:  # 前3次失败时打印错误
                    print("Actor %s: Gazebo model state service call failed: %s" % (self.id, e))
                
                # 如果持续失败，可能actor模型未正确加载，使用估算位置
                if self.service_fail_count > 10:
                    # 基于上次运动指令估算当前位置（简单积分）
                    dt = 1.0 / self.f  # 时间步长
                    if hasattr(self, 'last_avoid_cmd'):
                        self.current_pose.x += self.last_avoid_cmd.x * self.last_avoid_cmd.v * dt
                        self.current_pose.y += self.last_avoid_cmd.y * self.last_avoid_cmd.v * dt
                    # 保持在地图范围内
                    self.current_pose.x = max(self.x_min, min(self.x_max, self.current_pose.x))
                    self.current_pose.y = max(self.y_min, min(self.y_max, self.current_pose.y))
            ...
            if (self.avoid_finish_flag and self.distance_flag):
                while_time = 0  # 初始化重试计数器
                while self.suitable_point and while_time < 100:  # 添加最大重试限制
                    ...
                    for i in range(self.box_num):
                        if (self.x > self.black_box[i][0][0]-1.5) and (self.x < self.black_box[i][0][1]+1.5):
                            if (self.y > self.black_box[i][1][0]-1.5) and (self.y < self.black_box[i][1][1]+1.5):
                                ...
                                break
                    # 如果重试次数过多，强制选择一个远离障碍的位置
                    if while_time >= 50:
                        self.x = random.choice([self.x_min + 5, self.x_max - 5])
                        self.y = random.choice([self.y_min + 5, self.y_max - 5])
                        self.suitable_point = False
                        print(f"Actor {self.id}: 强制选择位置 ({self.x:.1f}, {self.y:.1f})")
                ...
                try:                   
                    ...
                    if (self.id == '5' or self.id == '4'):
                        ...
                        print('avoid_target:     '+ self.id+'      ', self.avoid)
                        print('\n')
                except Exception as e:
                    print(f"Actor {self.id} 路径规划失败: {e}")
                    # 简化的避障策略：如果在障碍物内，移动到最近的安全位置
                    ...
                    for i in range(len(self.black_box)):
                        if (self.current_pose.x > self.black_box[i][0][0]-1.0) and (self.current_pose.x < self.black_box[i][0][1]+1.0):
                            if (self.current_pose.y > self.black_box[i][1][0]-1.0) and (self.current_pose.y < self.black_box[i][1][1]+1.0):
                                # 计算到各边界的距离，选择最近的逃生方向
                                dis_list = [
                                    abs(self.current_pose.x - (self.black_box[i][0][0] - 1.0)),
                                    abs(self.current_pose.x - (self.black_box[i][0][1] + 1.0)),
                                    abs(self.current_pose.y - (self.black_box[i][1][0] - 1.0)),
                                    abs(self.current_pose.y - (self.black_box[i][1][1] + 1.0))
                                ]
                                dis_min = dis_list.index(min(dis_list))
                                self.subtarget_length = 1
                                if dis_min == 0:
                                    self.avoid.x = self.black_box[i][0][0] - 1.5
                                    self.avoid.y = self.current_pose.y
                                elif dis_min == 1:
                                    self.avoid.x = self.black_box[i][0][1] + 1.5
                                    self.avoid.y = self.current_pose.y
                                elif dis_min == 2:
                                    self.avoid.y = self.black_box[i][1][0] - 1.5
                                    self.avoid.x = self.current_pose.x
                                else:
                                    self.avoid.y = self.black_box[i][1][1] + 1.5
                                    self.avoid.x = self.current_pose.x
                                escape_success = True
                                break
                    ...
                    if(self.id == '5' or self.id == '4'):
                        print(self.id+' change position except - escape to avoid')
                        print('current_position:   ' + self.id+'      ', self.current_pose)
                        print('target_motion:     '+ self.id+'      ', self.target_motion)
                        print('avoid_target:     '+ self.id+'      ', self.avoid)
                        print('\n')
            ...
            '''
            # 确保运动指令合理性检查
            if abs(self.avoid.x) > 200 or abs(self.avoid.y) > 200:
                print(f"警告: Actor {self.id} 目标位置异常 ({self.avoid.x:.2f}, {self.avoid.y:.2f})，重置")
                self.avoid.x = self.current_pose.x
                self.avoid.y = self.current_pose.y
                self.distance_flag = True  # 重新生成目标
            
            # 限制速度避免过快运动导致的不稳定
            self.avoid.v = min(1.5, max(0.5, self.avoid.v))
            
            # 调试输出（限频）
            if self.count % 50 == 0:  # 每5秒打印一次
                print(f'Actor {self.id}: pos=({self.current_pose.x:.1f},{self.current_pose.y:.1f}), target=({self.avoid.x:.1f},{self.avoid.y:.1f}), v={self.avoid.v:.1f}')
            
            # 保存运动指令用于位置估算
            self.last_avoid_cmd = copy.deepcopy(self.avoid)
