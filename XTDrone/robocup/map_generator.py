...
## file path
output_path = os.path.expanduser(r'E:/robocup2025/PX4_Firmware/Tools/sitl_gazebo/worlds/')
rover_num = 20
...
# 预生成所有 actor 的位置，避免重复覆盖
actor_count = 6  # 假设有6个actor
actor_positions = []
for i in range(actor_count):
    a, b = create_human_point(black_box)
    actor_positions.append((a, b))
    print(f'生成 actor_{i} 位置: ({a}, {b})')

content = open("house.world", 'r')
rover_content = open("rover.world", 'r')
with open(output_path + "robocup1.world", 'w') as f:
    lines = content.readlines()
    ...
    for num, line in enumerate(lines):
        ...
        if 'filename="libros_actor_cmd_pose_plugin.so">' in line:
            # 找到下一行的 init_pose 并替换
            for next_line_idx in range(num + 1, min(num + 10, len(lines))):
                if '<init_pose>' in lines[next_line_idx] and actor_index < len(actor_positions):
                    a, b = actor_positions[actor_index]
                    lines[next_line_idx] = "        <init_pose>" + str(a) + ' ' + str(b) + " 1.25 1.57 0 0</init_pose>\n"
                    print(f'设置 actor_{actor_index} 位置: ({a}, {b})')
                    actor_index += 1
                    break
...
f.close()
print(f'human initial pose generated! 共设置 {actor_index} 个 actor 位置')
