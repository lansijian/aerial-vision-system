...
public:
    ...
    fixed_wing_formation_control::FWcmd cmd_from_controller; //FWcmd，来自各种控制器的四通道控制量

    //服务项暂存容器
    mavros_msgs::SetMode mode_cmd; /*创建mavros_msgs::SetMode类型的service消息*/

    mavros_msgs::CommandBool arming_service;/*创建mavros_msgs::CommandBool类型的service消息*/

    mavros_msgs::WaypointSetCurrent waypoint_set_current_service;/*创建mavros_msgs::WaypointSetCurrent类型的service消息*/

    mavros_msgs::WaypointPull waypoint_pull_service;/*创建mavros_msgs::WaypointPull类型的service消息*/

    mavros_msgs::WaypointPush waypoint_push_service;/*创建mavros_msgs::WaypointPush类型的service消息*/

    mavros_msgs::WaypointClear waypoint_clear_service;/*创建mavros_msgs::WaypointClear类型的service消息*/

    //发布的数据暂时容器
    mavros_msgs::PositionTarget local_pos_sp;/*创建mavros_msgs::PositionTarget类型的service消息*/

    mavros_msgs::GlobalPositionTarget global_pos_sp;/*创建mavros_msgs::GlobalPositionTarget类型的service消息*/


    mavros_msgs::AttitudeTarget att_sp;  /*【发布AttitudeTarget】姿态、角速度和油门设定值的暂时容器*/

    fixed_wing_formation_control::FWstates fw_states_form_mavros; //FWstates,自定义的完整的飞机状态消息
