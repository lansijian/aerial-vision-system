...
void SWITCH_FW_MODE::run() {
  ...
  ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>(              /* 【订阅】无人机当前状态 */
      add2str(uavID, "mavros/state"), 10, &SWITCH_FW_MODE::state_cb, this);  /* arm预备，manual手动等*/

  
  ros::Publisher task_cmd_mode_pub =                                         /* 【发布】编队控制器期望状态 */
      nh.advertise<fixed_wing_formation_control::Fw_cmd_mode>(               /* -------由task_main.cpp订阅*/
          add2str(uavID, "fixed_wing_formation_control/fw_cmd_mode"), 10);

  
  ros::ServiceClient arm_client =                                            /* 【服务】 修改锁定状态 */
      nh.serviceClient<mavros_msgs::CommandBool>(add2str(uavID, "mavros/cmd/arming"));/*-----pack_fw_states.cpp也有声明*/

  
  ros::ServiceClient set_mode_client =                                       /* 【服务】 修改系统模式 */
      nh.serviceClient<mavros_msgs::SetMode>(add2str(uavID, "mavros/set_mode"));/*-----pack_fw_states.cpp也有声明*/
