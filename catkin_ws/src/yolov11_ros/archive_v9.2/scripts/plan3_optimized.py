...
class OptimizedVisualServo:
    ...
    def track_target(self):
        ...
        # 加速模式：距离>3.5米 或 人在加速且距离>3.2米（但不能打断减速）
        ...
        if self.tracking_mode == "STABLE":
            ...
            if abs(u) < 150:
                ...
                # 左下方：后退+左转
                self.x_velocity = -1.0
                self.yaw_rate = 0.3  # 左转
            else:
                # 右下方：后退+右转
                self.x_velocity = -1.0
                self.yaw_rate = -0.3  # 右转
        ...
        # ==================== 4. 转向控制（让人居中） ====================
        if self.tracking_mode not in ["RETREAT", "RETREAT_FAST"]:  # 后退模式单独处理yaw
            abs_u = abs(u)
        ...
        if self.tracking_count % 15 == 0:
            ...
            rospy.loginfo(f"[{mode_str}][{color_name}] 距离={estimated_distance:.1f}m({distance_level}) "
                         f"人速={self.person_speed:.1f}m/s {behind_str} | "
    ...
    def handle_target_lost(self):
        ...
        处理目标丢失：判断搜索策略
        """
        # 优先判断：如果之前在快速后退（人快速接近），继续后退
        if self.tracking_mode == "RETREAT_FAST":
            self.search_strategy = "RETREAT_FAST"
            rospy.logwarn(f"⚠️  快速接近中丢失目标！→ 继续快速后退")
            return
        
        # 判断消失位置
        in_left = self.last_u < -150
        in_right = self.last_u > 150
        in_center_x = abs(self.last_u) < 150
        in_upper = self.last_v < self.image_height * 0.35
        in_lower = self.last_v > self.image_height * 0.65
        in_middle_y = not in_upper and not in_lower
        
        # 判断距离
        estimated_distance = self.estimate_distance_from_box_size(self.last_box_size)
        is_far = estimated_distance > 4.0
        ...
        elif in_center_x and in_middle_y:
            self.search_strategy = "HOVER"
            rospy.logwarn(f"⚠️  目标从中央消失！→ 悬停等待（可能遮挡）")
        else:
            self.search_strategy = "HOVER"
            rospy.logwarn(f"⚠️  目标消失！→ 悬停观察")
