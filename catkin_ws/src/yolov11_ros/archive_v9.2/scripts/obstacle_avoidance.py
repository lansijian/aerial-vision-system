...
class ElegantObstacleAvoidance:
    def __init__(self):
        ...
        self.avoidance_status_pub = rospy.Publisher(
            f'/drone_{self.drone_id}/obstacle_avoidance/status', String, queue_size=1
        )
        self.avoidance_active_pub = rospy.Publisher(
            f'/drone_{self.drone_id}/obstacle_avoidance/active', Bool, queue_size=1
        )
        self.flight_mode_pub = rospy.Publisher(
            f'/drone_{self.drone_id}/obstacle_avoidance/flight_mode', String, queue_size=1
        )
        ...
        # 监听系统状态（非侵入式状态推断，多机话题命名）
        rospy.Subscriber(f'/drone_{self.drone_id}/human_tracker/status', Bool, 
                        self._tracking_status_callback, queue_size=1)
        rospy.Subscriber(f'/drone_{self.drone_id}/waypoint_flight/status', String, 
        ...
        rospy.loginfo("="*60)
        rospy.loginfo(f"🛡️ 优雅避障系统已启动 - 无人机 {self.drone_id}")
        rospy.loginfo("="*60)
        rospy.loginfo(f"   无人机ID: {self.drone_id}")
        rospy.loginfo(f"   无人机总数: {self.num_drones}")
        rospy.loginfo(f"   命名空间: {vehicle_ns}")
    ...
    def run(self):
        ...
        rospy.loginfo(f"🚁 避障系统开始监控... - 无人机 {self.drone_id}")
