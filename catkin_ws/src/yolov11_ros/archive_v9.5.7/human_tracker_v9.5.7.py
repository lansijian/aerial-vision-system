...
"""
Human Tracker v9.5.7 - 修复坐标转换问题的版本

主要修复：
1. 修正相机参数（cy=180.5, image_height=360）
2. 处理四元数符号问题
3. 完整的云台旋转矩阵（pitch, yaw, roll）
4. 添加坐标系偏移补偿
5. 参数可配置化
...
class HumanTracker:
    def __init__(self):
        ...
        rospy.loginfo(f"✅ 无人机{self.drone_id}追踪器初始化完成 (v9.5.7)")
    ...
    def _publish_actor_info(self, target):
        ...
        # 4. 使用地面约束计算交点
        target_height = self.target_height  # 使用可配置的人体重心高度
        ...
        if abs(ray_enu[2]) < 0.01:  # 射线几乎水平，无法与地面相交
            ...
            if t <= 0:  # 射线向上，无法与地面相交
                ...
                rospy.logwarn_throttle(5.0, f"[警告] 计算距离异常: {t:.2f}m，使用估计值")
                t = estimated_distance
            else:
