...
"""
Human Tracker v10.0.1-fixed - 修复版
基于v10.0.1存档，修复了追踪模式切换问题
...
class HumanTracker:
    def __init__(self):
        ...
        self.cmd_vel = Twist()
        self.flight_mode = "IDLE"  # 添加飞行模式状态
        ...
        rospy.loginfo("=" * 60)
        rospy.loginfo(f"✅ 无人机{self.drone_id} 追踪器v10.0.1-fixed初始化完成")
        ...
        rospy.loginfo(f"   【修复版】话题名称已修正，添加模式监听")
        rospy.loginfo("=" * 60)
    ...
    def _handle_no_detection(self):
        ...
        if self.lost_count > 30:  # 1秒无检测
            if self.tracking_active:
                rospy.loginfo(f"⚠️ 无人机{self.drone_id}: 1秒未检测到目标，恢复航点模式")
                self._stop_tracking()
        else:
            # 短暂丢失，保持减速
    ...
    def run(self):
        """主循环"""
        rate = rospy.Rate(30)
        rospy.loginfo(f"🚀 无人机{self.drone_id} 追踪器v10.0.1-fixed开始运行")
